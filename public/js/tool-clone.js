...
class CloneTool extends Tool {
    constructor() {
        ...
        this.sourcePos = { x: undefined, y: undefined };
    ...
    onMouse(event) {
        if (CustomMouseEvent.mouseRightDown && CustomMouseEvent.mouseRightChanged) {
            ...
            if (CustomMouseEvent.mouseLeftDown) {
                this.blit(DrawView.getLayer(0).drawing, DrawView.getLayer(0).drawing, this.sourcePos.x - this.conf["Size"].value + (Util.screenToCordX(CustomMouseEvent.mouseX) - this.targetPos.x), this.sourcePos.y - this.conf["Size"].value + (Util.screenToCordY(CustomMouseEvent.mouseY) - this.targetPos.y), this.conf["Size"].value * 2 + 1, this.conf["Size"].value * 2 + 1, Util.screenToCordX(CustomMouseEvent.mouseX) - this.conf["Size"].value, Util.screenToCordY(CustomMouseEvent.mouseY) - this.conf["Size"].value);
            }
